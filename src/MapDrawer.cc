...
void MapDrawer::DrawDepthPoints() {
    ...
    for (uint i = 0; i < viewPoints.size(); ++i) {
        Eigen::Vector3f color = colors[i].cast<float>();

        glColor3f(color.x(), color.y(), color.z());
        glPointSize(3.0);
        glBegin(GL_POINTS);

        const Eigen::Vector3f p = viewPoints[i].cast<float>();
...
void MapDrawer::SetDepthColorPoints(std::vector<Eigen::Vector3d> &viewPoints_, std::vector<Eigen::Vector3d> &colors_) {
