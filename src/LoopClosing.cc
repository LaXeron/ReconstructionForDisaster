...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, shared_ptr<PointCloudMapping> pPointCloud):
    ...
    mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mpPointCloudMapping( pPointCloud )
{
...
void LoopClosing::printReprojectionError(set<KeyFrame*> &spLocalWindowKFs, KeyFrame* mpCurrentKF, string &name)
{
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        cv::Mat img_i = cv::imread(pKFi->mNameFile, -1);
        ...
        if(!mbStopGBA)
        {
            ...
            while(!lpKFtoCheck.empty())
            {
                ...
                if(dist > 1)
                {
                    ...
                    cv::Mat imLeft = cv::imread(strNameFile, -1);
