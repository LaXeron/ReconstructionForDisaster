...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas,
    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpAtlas(pAtlas), mpPointCloudMapping(pPointCloud),
    mnLastRelocFrameId(0), time_recently_lost(5.0),
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        std::cout<<"============  set Point Map =================="<<std::endl;

        if(mpPointCloudMapping->getPointCloudMapUpdatedFlag()){
            vector<Eigen::Vector3d> depthmap;
            vector<Eigen::Vector3d> colormap;

            
            mpPointCloudMapping->GetDepthColorMap(depthmap, colormap);

            std::cout<<"depth map size "<<depthmap.size()<<std::endl;

            mpMapDrawer->SetDepthColorPoints(depthmap, colormap);
        }
...
void Tracking::CreateNewKeyFrame()
{
    ...
    std::cout<<"insertKeyFrame cost time is "<<dr_s<<std::endl;
