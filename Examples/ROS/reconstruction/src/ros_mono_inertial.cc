...
cv::Mat getImageFromMsg(const sensor_msgs::ImageConstPtr &img_msg)
{
  cv::Mat mat;
    ...
    if(image_msg->encoding == "bgr8" || image_msg->encoding == "rgb8"){
        mat = cv::Mat(image_msg->height, image_msg->width, CV_8UC3, const_cast<uchar*>(rawImageData)).clone();
    } else {
        mat = cv::Mat(image_msg->height, image_msg->width, CV_8UC1, const_cast<uchar*>(rawImageData)).clone();
    }
    return mat;
}
